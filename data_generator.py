...
from utils import get_var, plot_scatter
...
zv_prior[0] = constant(0.1*np.ones((sample_size,2)))
zm_prior[1] = constant(-1*np.ones((sample_size,2)))
zv_prior[1] = constant(0.1*np.ones((sample_size,2)))
for i in range(k):
...
with tf.Session() as sess:
    ...
    plot_scatter(z, y, 'scatter_true_z.png')
    plot_scatter(x, y, 'scatter_x.png')
